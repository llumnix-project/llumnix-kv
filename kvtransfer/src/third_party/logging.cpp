// copy from: https://github.com/alibaba/BladeDISC/blob/main/tao_compiler/mlir/ral/ral_logging.cc
#include <stdlib.h>
#include <string.h>
#include <time.h>
#include <unistd.h>
...
VmoduleMap *VmodulesMapFromEnv() {
  ...
  //    "foo=1,bar=2,baz=3"
  const char *env = getenv("LLMOPS_CPP_VMODULE");
  if (env == nullptr) {
    // If there is no LLMOPS_CPP_VMODULE configuration (most common case), return
...
int MinLogLevelFromEnv() {
  const char *tf_env_var_val = getenv("LLMOPS_CPP_MIN_LOG_LEVEL");
  return LogLevelStrToInt(tf_env_var_val);
}

int MinVLogLevelFromEnv() {
  const char *tf_env_var_val = getenv("LLMOPS_CPP_MIN_VLOG_LEVEL");
  return LogLevelStrToInt(tf_env_var_val);
}
...
bool LogMessage::VmoduleActivated(const char *fname, int level) {
  ...
  if (PREDICT_TRUE(vmodules == nullptr)) {
    return false;
  }
