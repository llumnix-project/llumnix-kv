...
KV transfer 涉及两个语义不同的 "TP size"，统一约定如下：

| 名称 | 存储位置 | 含义 | 何时计算 |
|------|----------|------|----------|
| `engine_tp_size` | `WorkerInfo::engine_tp_size`（序列化） | 模型引擎真实的 tensor-parallel size，对 P/D 双方都是配置值 | 进程初始化时由 `Context::set_tp` 写入 |
| `kvt_tp_size` (effective) | `KvSendStub::TaskContext::kvt_tp_size`（**不**在 `WorkerInfo` 中） | 经 `valid_ranks` 过滤后实际参与传输的 P 侧 rank 数 = `valid_ranks.count()` | `refresh_dst_info()` 中按目的端动态计算 |
| `kvt_tp_rank` | `KvSendStub::TaskContext::kvt_tp_rank` | 当前 src 的 `worker_tp_rank` 在 `valid_ranks` 中的序号 | 同上 |

`valid_ranks` 由 `compute_valid_ranks_pd(p_engine_tp, d_engine_tp, num_kv_heads)` 计算，
反映"当 `num_kv_heads < engine_tp_size` 时只有部分 P rank 真正持有数据"这一事实，
仅影响 **attention** 路径；GDN / indexer 的分组是 engine-tp-based，不受 `valid_ranks` 影响。

### parse_block / build_transfer_plan 中的 TP 选择

`tx_stub.cpp::selection_p_tp()` 提供统一的选择规则：

```cpp
// 仅对带 GDN 的 cache shape，用 engine_tp_size 选择 P==D / P>D 分支；
// 其余 shape 用 kvt_tp_size（effective）选择。
selection_p_tp(cache_shape, src, effective_kvt_tp_size)
  = (cache_shape ∈ {QWEN3_NEXT_FLASH, QWEN3_NEXT_FLASHINFER})
      ? src.engine_tp_size
      : effective_kvt_tp_size
```

判定 P>D / P==D / P<D 时与之比较的 dst TP **统一为** `dst.engine_tp_size`。
`build_transfer_plan` 内部使用同样的规则（`p_tp_for_selection = has_gdn ? src.engine_tp_size : kvt_tp_size`），
因此两条路径选出的分支永远一致。

### parse_block_* 函数参数

`parse_block_*` 系列函数签名固定为：

```cpp
void parse_block(const WorkerInfo* src, const WorkerInfo* dst,
                 const std::bitset<MAX_TP_SIZE>& valid_ranks,
                 uint32_t kvt_tp_size, uint32_t kvt_tp_rank,
                 const ReqSendTask* task,
                 std::vector<std::vector<IpcBlock>>& send_blocks);
```

约束：
- 函数内 P 侧的有效 TP 一律读取参数 `kvt_tp_size`，**不要**再去读 `src->...`。
- 函数内 D 侧 TP 一律读取 `dst->engine_tp_size`。
- 对 QWEN3_NEXT 的 GDN 分组使用 `src->engine_tp_size / dst->engine_tp_size`；
  对 attention 的分组使用 `valid_ranks.count() / dst->engine_tp_size`。
- 处理 P<D 的 helper（`do_parse_block_send_internal` / `do_multi_tensor_parse_block_send`）需要 P 与 D 互换调用：
  显式接受 `p_tp_size` / `d_tp_size` 两个参数，由调用方传入 effective 值，helper 内部不再读 WorkerInfo 中的 TP 字段。
