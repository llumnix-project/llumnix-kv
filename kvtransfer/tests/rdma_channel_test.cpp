...
#define NAMING_DIR "./rdma_test_naming"
#ifdef ENABLE_RDMA
...
TEST(RDMAChannelTest, TestTransfer) {
  ...
  std::filesystem::remove_all(NAMING_DIR);
  FileSysNaming::create_dir(NAMING_DIR);
  auto pid = fork();
  if (pid > 0) {
    ...
    FileSysNaming fn("0");
    fn.connect(FILESYS_NAMING_SCHEMA, NAMING_DIR);
    WorkerNamingClient worker_client(&fn);
    worker_client.register_worker(ctx.worker_info());
    ...
    fn.connect(FILESYS_NAMING_SCHEMA, NAMING_DIR);
    WorkerNamingClient worker_client(&fn);
    auto dst_info = worker_client.get_worker_info("0", 0);
    size_t retry_cnt = 3;
    while (!dst_info.has_value() && retry_cnt > 0) {
      std::this_thread::sleep_for(std::chrono::milliseconds(100));
      dst_info = worker_client.get_worker_info("0", 0);
      retry_cnt--;
    }
